...
import 'notifier/dr_details_notifier.dart';
...
import 'package:the_6_04_health_care/widgets/custom_icon_button.dart';

class DrDetailsScreen extends ConsumerStatefulWidget {
...
class DrDetailsScreenState extends ConsumerState<DrDetailsScreen> {
  @override
        ...
        child: Consumer(builder: (context, ref, _) {
          return SizedBox(
              ...
              child: TableCalendar(
                  ...
                  rangeSelectionMode:
                      ref.watch(drDetailsNotifier).rangeSelectionMode,
                  ...
                  headerVisible: false,
                  focusedDay:
                      ref.watch(drDetailsNotifier).focusedDay ?? DateTime.now(),
                  rangeStartDay: ref.watch(drDetailsNotifier).rangeStart,
                  rangeEndDay: ref.watch(drDetailsNotifier).rangeEnd,
                  onDaySelected: (selectedDay, focusedDay) {
                    if (!isSameDay(ref.watch(drDetailsNotifier).selectedDay,
                        selectedDay)) {
                      ref.watch(drDetailsNotifier).focusedDay = focusedDay;
                      ref.watch(drDetailsNotifier).selectedDay = selectedDay;
                      ref.watch(drDetailsNotifier).rangeSelectionMode =
                  ...
                  onRangeSelected: (start, end, focusedDay) {
                    ref.watch(drDetailsNotifier).focusedDay = focusedDay;
                    ref.watch(drDetailsNotifier).rangeEnd = end;
                    ref.watch(drDetailsNotifier).rangeStart = start;
                    ref.watch(drDetailsNotifier).rangeSelectionMode =
                        RangeSelectionMode.toggledOn;
                  },
    ...
    return Consumer(builder: (context, ref, _) {
      return Wrap(
          ...
          children: List<Widget>.generate(
              ref
                      .watch(drDetailsNotifier)
                      .drDetailsModelObj
                      ?.timeslotsItemList
                      .length ??
                  0, (index) {
            TimeslotsItemModel model = ref
                    .watch(drDetailsNotifier)
                    .drDetailsModelObj
            ...
            return TimeslotsItemWidget(model, onSelectedChipView1: (value) {
              ref
                  .read(drDetailsNotifier.notifier)
