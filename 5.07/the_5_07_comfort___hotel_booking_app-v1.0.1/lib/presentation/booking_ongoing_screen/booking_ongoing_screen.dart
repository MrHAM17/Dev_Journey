...
/// Section Widget
Widget _buildTickets(BuildContext context) { return Consumer<BookingOngoingProvider>(builder: (context, provider, child) {return ListView.separated(physics: NeverScrollableScrollPhysics(), shrinkWrap: true, separatorBuilder: (context, index) {return Padding(padding: EdgeInsets.symmetric(vertical: 10.0.v), child: SizedBox(width: 340.h, child: Divider(height: 1.v, thickness: 1.v, color: appTheme.gray700)));}, itemCount: provider.bookingOngoingModelObj.tickets1ItemList.length, itemBuilder: (context, index) {Tickets1ItemModel model = provider.bookingOngoingModelObj.tickets1ItemList[index]; return Tickets1ItemWidget(model, onTapCancelBooking: () {onTapCancelBooking(context);}, onTapViewTicket: () {onTapViewTicket(context);});});}); } 
/// Section Widget
