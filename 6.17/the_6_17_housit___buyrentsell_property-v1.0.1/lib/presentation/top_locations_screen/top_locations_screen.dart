...
/// Section Widget
Widget _buildData(BuildContext context) { return Expanded(child: Consumer(builder: (context, ref, _) {return GridView.builder(shrinkWrap: true, gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(mainAxisExtent: 227.v, crossAxisCount: 2, mainAxisSpacing: 7.h, crossAxisSpacing: 7.h), physics: BouncingScrollPhysics(), itemCount: ref.watch(topLocationsNotifier).topLocationsModelObj?.dataItemList.length ?? 0, itemBuilder: (context, index) {DataItemModel model = ref.watch(topLocationsNotifier).topLocationsModelObj?.dataItemList[index] ?? DataItemModel(); return DataItemWidget(model, onTapEstatesCardLocation: () {onTapEstatesCardLocation(context);});});})); } 
