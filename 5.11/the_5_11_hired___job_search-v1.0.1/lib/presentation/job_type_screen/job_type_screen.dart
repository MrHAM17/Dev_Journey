...
import 'provider/job_type_provider.dart';

class JobTypeScreen extends StatefulWidget {
  const JobTypeScreen({Key? key}) : super(key: key);

  @override
  JobTypeScreenState createState() => JobTypeScreenState();

  static Widget builder(BuildContext context) {
    return ChangeNotifierProvider(
        create: (context) => JobTypeProvider(), child: JobTypeScreen());
  }
}

class JobTypeScreenState extends State<JobTypeScreen> {
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget _buildJobType(BuildContext context) {
    return SizedBox(
        height: 229.v,
        child: Consumer<JobTypeProvider>(builder: (context, provider, child) {
          return ListView.separated(
              scrollDirection: Axis.horizontal,
              separatorBuilder: (context, index) {
                return SizedBox(width: 15.h);
              },
              itemCount: provider.jobTypeModelObj.jobtypeItemList.length,
