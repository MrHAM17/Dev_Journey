...
import 'provider/explore_eight_provider.dart';

class ExploreEightPage extends StatefulWidget {
  const ExploreEightPage({Key? key}) : super(key: key);

  @override
  ExploreEightPageState createState() => ExploreEightPageState();

  static Widget builder(BuildContext context) {
    return ChangeNotifierProvider(
        create: (context) => ExploreEightProvider(), child: ExploreEightPage());
  }
}

class ExploreEightPageState extends State<ExploreEightPage> {
  @override
  void initState() {
    super.initState();
  ...
  Widget _buildSpecials(BuildContext context) {
    return SizedBox(
        height: 60.v,
        child:
            Consumer<ExploreEightProvider>(builder: (context, provider, child) {
          return ListView.separated(
              scrollDirection: Axis.horizontal,
              separatorBuilder: (context, index) {
                return SizedBox(width: 16.h);
              },
              itemCount: provider.exploreEightModelObj.specials7ItemList.length,
              itemBuilder: (context, index) {
                Specials7ItemModel model =
                    provider.exploreEightModelObj.specials7ItemList[index];
  ...
  Widget _buildExploreEight(BuildContext context) {
    return Consumer<ExploreEightProvider>(builder: (context, provider, child) {
      return GridView.builder(
          ...
          itemCount: provider.exploreEightModelObj.exploreeightItemList.length,
          itemBuilder: (context, index) {
            ExploreeightItemModel model =
                provider.exploreEightModelObj.exploreeightItemList[index];
