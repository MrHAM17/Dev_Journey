...
import 'notifier/doctor_detail_notifier.dart';
...
class DoctorDetailScreenState extends ConsumerState<DoctorDetailScreen> {
  @override
      ...
      Consumer(builder: (context, ref, _) {
        return SizedBox(
            ...
            child: TableCalendar(
                ...
                rangeSelectionMode:
                    ref.watch(doctorDetailNotifier).rangeSelectionMode,
                ...
                headerVisible: false,
                focusedDay: ref.watch(doctorDetailNotifier).focusedDay ??
                    DateTime.now(),
                rangeStartDay: ref.watch(doctorDetailNotifier).rangeStart,
                rangeEndDay: ref.watch(doctorDetailNotifier).rangeEnd,
                onDaySelected: (selectedDay, focusedDay) {
                  if (!isSameDay(ref.watch(doctorDetailNotifier).selectedDay,
                      selectedDay)) {
                    ref.watch(doctorDetailNotifier).focusedDay = focusedDay;
                    ref.watch(doctorDetailNotifier).selectedDay = selectedDay;
                    ref.watch(doctorDetailNotifier).rangeSelectionMode =
                ...
                onRangeSelected: (start, end, focusedDay) {
                  ref.watch(doctorDetailNotifier).focusedDay = focusedDay;
                  ref.watch(doctorDetailNotifier).rangeEnd = end;
                  ref.watch(doctorDetailNotifier).rangeStart = start;
                  ref.watch(doctorDetailNotifier).rangeSelectionMode =
                      RangeSelectionMode.toggledOn;
                },
      ...
      Consumer(builder: (context, ref, _) {
        return Wrap(
            ...
            children: List<Widget>.generate(
                ref
                        .watch(doctorDetailNotifier)
                        .doctorDetailModelObj
                        ?.amItemList
                        .length ??
                    0, (index) {
              AmItemModel model = ref
                      .watch(doctorDetailNotifier)
                      .doctorDetailModelObj
                      ?.amItemList[index] ??
                  AmItemModel();
              return AmItemWidget(model, onSelectedChipView1: (value) {
                ref
                    .read(doctorDetailNotifier.notifier)
